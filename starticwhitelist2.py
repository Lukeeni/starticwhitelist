...
# Load Outseta credentials from secrets
client_id = st.secrets["OUTSETA_CLIENT_ID"]
client_secret = st.secrets["OUTSETA_CLIENT_SECRET"]
subdomain = st.secrets["OUTSETA_SUBDOMAIN"]

@st.cache_data(ttl=300)  # Cache email list for 5 minutes
def get_approved_emails():
    url = f"https://{subdomain}.outseta.com/api/v1/crm/accounts"
    response = requests.get(url, auth=HTTPBasicAuth(client_id, client_secret))
    if response.status_code != 200:
        st.error("Could not connect to Outseta. Try again later.")
        return []

    emails = []
    data = response.json()
    for account in data.get("items", []):
        for person in account.get("People", []):
            if "Email" in person:
                emails.append(person["Email"].lower())
    return emails
...
if not email:
    st.error("No email provided. Access denied.")
    st.stop()

approved_emails = get_approved_emails()

if email.lower() not in approved_emails:
    st.error("You are not an approved user. Access denied.")
    st.stop()
